...
int main(int argc, char* argv[]) {
	makeSound(&sound_start, "C:/Users/tytgsjsn/Desktop/Visual Studio/C Project/Termily/sounds/start.wav");
    ...
    while (1) {
        ...
        if (checkKeyDown(&key_up)) player.y--;
        if (checkKeyDown(&key_down)) player.y++;
        if (checkKeyDown(&key_left)) player.x--;
        if (checkKeyDown(&key_right)) player.x++;

		if (checkMouseDown(&mouse_left)) break;
